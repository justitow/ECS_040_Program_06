...
if grep "include *<" *.cpp *.h | egrep -v "(iostream|cstring|fstream)" &> /dev/null ; then
  echo 'Standard library other than iostream, cstring, and fstream found.'  >> designResults.
  (( ++ designError ))
fi
...
if ! grep "bool *find *(" list.h  &> /dev/null  &> /dev/null ; then
  echo "bool find( not found in list.h" >> designResults.txt
fi
...
if ! grep -v "//" directory.cpp | grep "File *:: *File"  &> /dev/null  ; then
  echo "File constructor not found in directory.cpp" >> designResults.txt
fi

if ! grep -v "//" directory.cpp | grep "File *:: *getName"  &> /dev/null  ; then
  echo "File::getName not found in directory.cpp" >> designResults.txt
fi
...
if ! grep -v "//" directory.h | awk '/class *File *([^;]|$)/,/};/' | grep "virtual *bool *operator *=="  &> /dev/null  ; then
  echo "File operator== not found in directory.h" >> designResults.txt
fi
...
if ! grep -v "//" directory.h | awk '/class *Directory *([^;]|$)/,/};/' | grep " *bool *operator *=="  &> /dev/null  ; then
  echo "Directory operator== not found in directory.h" >> designResults.txt
fi
